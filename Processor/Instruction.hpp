...
inline void Instruction::execute(Processor<sint, sgf2n> &Proc) const
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        case PUBOUTPUT:
        {
            auto temp = Proc.template get_input<IntInput<typename sint::clear>>(
                Proc.public_file, Proc.public_output_filename, 0);
            // cout << temp.items[0] << endl;
            Proc.get_Cp_ref(r[0]) = temp.items[0];
            // cout << Proc.get_C(r[0]) << endl;
            break;
        }
        case RAWOUTPUT:
