...
public:
    ...
    OTCorrelator(BaseOT &baseOT, TwoPartyPlayer *player, bool passive) : OTExtension(baseOT, player, passive),
                                                                         senderOutputMatrices(2), matrices(2),
                                                                         receiverOutputMatrix(matrices[0]), t1(matrices[1]) {}
...
public:
    ...
    void print_sender(int n);

    octet *get_receiver_output(int i);
    octet *get_sender_output(int choice, int i);
    __m128i get_receiver_output128i(int i);
    void get_sender_output128i(std::array<vector<BitVector>, 2> &sender_output, int begin, int end);
