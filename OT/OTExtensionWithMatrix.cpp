...
void OTExtensionWithMatrix::soft_sender(size_t n)
{
    ...
    // {
    //     for (int j = 0; j < 2; j++)
    //         std::cout << msg.assign_bytes((char *)(sendMessages[i][j]), sizeof(__m128i)) << " ";
    //     std::cout << std::endl;
    // }

    for (size_t i = 0; i < n; i++)
        for (int j = 0; j < 2; j++)
            senderOutputMatrices[j]
                .squares.at(i / 128)
                .rows[i % 128] =
                sendMessages[i][j];
}
...
void OTExtensionWithMatrix::print_sender(int n)
{
    BitVector sender_output;
    for (int j = 0; j < n; j++)
    {
        for (int i = 0; i < 2; i++)
        {
            sender_output.assign_bytes((char *)get_sender_output(i, j), sizeof(__m128i));
